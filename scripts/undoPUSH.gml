...
for(var i = 0; i < instSize; i += 1){
    ...
    if _inst.object_index == oKina{
        ...
        ds_stack_push(undoStack,_inst.glitchMimic);
        ...
        _bin += _digit*_inst.browned; _digit *= 2;
        _bin += _digit*numBinDigit(_inst.glitchMimic,3); _digit *= 2;
        _bin += _digit*numBinDigit(_inst.glitchMimic,2); _digit *= 2;
        _bin += _digit*numBinDigit(_inst.glitchMimic,1); _digit *= 2;
        _bin += _digit*numBinDigit(_inst.glitchMimic,0); _digit *= 2;
        if _inst.copies >= 0{_bin += _digit;} _digit *= 2;
        for(var ii = 0; ii < 26; ii += 1){
            _bin += _digit*numBinDigit(_inst.copies,ii); _digit *= 2;
        }
        ...
        _bin += _digit*numBinDigit(_inst.glitchMimic,2); _digit *= 2;
        _bin += _digit*numBinDigit(_inst.glitchMimic,1); _digit *= 2;
        _bin += _digit*numBinDigit(_inst.glitchMimic,0);
        //ds_stack_push(undoStack,_inst.active,_inst.glitchMimic);
        ds_stack_push(undoStack,_bin);
        //Saves a 5-byte number storing both the active bool and glitchMimic.
        ...
        //ds_stack_push(undoStack,_inst.active,_inst.aura[0],_inst.aura[1],_inst.aura[2],_inst.browned,_inst.copies,_inst.glitchMimic);
        ds_stack_push(undoStack,_bin);
        //Saves 9 bits of bool and 64 bits of copies. Can be 9 bools, 27 copies, 27 icopies. Won't be reached ingame.
