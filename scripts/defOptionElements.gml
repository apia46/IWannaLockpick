///defOptionElements();
//Defines all the options menu elements.
//Buttons are rectangles which display either a sprite OR title text inside.

//Start with the "constants" for each tab.
for(var i = 0; i < numTabs; i += 1){
    for (var ii = 0; ii < numTabs; ii += 1){
        defOptionButton(i,ii,400-64+tabSpacing*(ii-2),tabBY,tabBW,32,-1,-1,tabName[ii],tabDesc[ii]);
    }
    defOptionButton(i,numTabs,400-192,560,160,32,-1,-1,"Save and Return","Save the current config and return to the Pause Menu.");
    if room == rTitleNew{oButName[i,numTabs] = "Save and Begin"; oButDesc[i,numTabs] = "Save the current config and begin the game.";}
    defOptionButton(i,numTabs+1,400+32,560,160,32,-1,-1,"Restore Defaults","Restores this tab's original settings.");
}
for (var ii = 0; ii < numTabs; ii += 1){//Paths
    var _lt = ii - 1; if ii == 0{_lt = 4;}
    var _rt = ii + 1; if ii == (numTabs-1){_rt = 0;}
    //Top buttons
    ...
    //Bottom buttons
    var _bottomID = -1; var _bottomIsSld = -1;
    if ii == 0{_bottomID = 2; _bottomIsSld = 1;}
    if ii == 1{_bottomID = numTabs+6; _bottomIsSld = 0;}
    if ii == 2{_bottomID = 8; _bottomIsSld = 1;}
    if ii == 3{_bottomID = numTabs+6; _bottomIsSld = 0;}
    if ii == 4{_bottomID = numTabs+7; _bottomIsSld = 0;}
...
var _fb = numTabs+2;//First non-"main" Button ID. Number of tabs + "back" and "restore"
var _spr = -1;//Sprite for buttons.
var _ind = -1;//Index for buttons.
var _baseX = 32;//Left alignment for buttons.
var _baseY = 160;//First y value "underneath" the tab buttons and name/description.

//SOUND
if global.muteMusic{_ind = 3;}else{_ind = 2;}
defOptionButton(0,_fb,_baseX,_baseY,32,32,sprOptionSymbols,_ind,"Mute Music","Ingame music will not play. Sounds unaffected.");
...
//VISUAL (Needs to add "pause style" and "window scale", whatever those mean...
if global.fullscreenMode{_ind = 1;}else{_ind = 0;}
defOptionButton(1,_fb,_baseX,_baseY,32,32,sprOptionSymbols,_ind,"Enable Fullscreen","Enables Fullscreen.");
defOptionPaths(1,_fb,0,0,0,_fb+1,0,-1,-1,-1,-1);
if global.smoothingMode{_ind = 1;}else{_ind = 0;}
defOptionButton(1,_fb+1,_baseX,_baseY+40,32,32,sprOptionSymbols,_ind,"Smoothing Mode","Enables smooth texture interpolation.");
defOptionPaths(1,_fb+1,0,_fb,0,_fb+2,0,-1,-1,-1,-1);
if global.vsyncMode{_ind = 1;}else{_ind = 0;}
defOptionButton(1,_fb+2,_baseX,_baseY+80,32,32,sprOptionSymbols,_ind,"Enable VSync","Enables VSync. If you don't know what that is, ignore it.");
defOptionPaths(1,_fb+2,0,_fb+1,0,_fb+3,0,-1,-1,-1,-1);
if global.lowFX{_ind = 1;}else{_ind = 0;}
defOptionButton(1,_fb+3,_baseX,_baseY+120,32,32,sprOptionSymbols,_ind,"Low FX","Removes certain graphical effects. Use to reduce lag or visual noise.");
//defOptionPaths(1,_fb+3,0,_fb+2,0,numTabs,0,-1,-1,-1,-1);
defOptionPaths(1,_fb+3,0,_fb+2,0,_fb+4,0,-1,-1,-1,-1);
if global.simpleLock{_ind = 1;}else{_ind = 0;}
defOptionButton(1,_fb+4,_baseX,_baseY+160,32,32,sprOptionSymbols,_ind,"Simple Locks","Forces numerical lock sprites. If you have trypophobia, I advise using it.");
defOptionPaths(1,_fb+4,0,_fb+3,0,numTabs,0,-1,-1,-1,-1);

//COLORBLIND
//Change color
defOptionButton(2,_fb,_baseX+48,_baseY+12,32,32,sprOptionSymbols,4,"","Edit the previous color.");
defOptionPaths(2,_fb,0,0,0,0,1,-1,-1,_fb+1,0);
defOptionButton(2,_fb+1,_baseX+48+40,_baseY+12,32,32,sprOptionSymbols,5,"","Edit the next color.");
defOptionPaths(2,_fb+1,0,0,0,0,1,_fb,0,_fb+2,0);
//3 sliders, RGB, for light/main/dark tones
_baseX += 48;
...
_baseX -= 48;
//Set light/dark tones to main tone
defOptionButton(2,_fb+2,600-128,_baseY+160+32,256,32,-1,-1,"Set All Tones to Main","Sets the light and dark tones to the main tone.");
defOptionPaths(2,_fb+2,0,0,0,_fb+3,0,_fb,0,-1,-1);
//Presets
defOptionButton(2,_fb+3,600-128,_baseY+160+32+40,256,32,-1,-1,"Use Preset: Default","Use the basic color settings.");
defOptionPaths(2,_fb+3,0,_fb+2,0,_fb+4,0,_fb,0,-1,-1);
defOptionButton(2,_fb+4,600-128,_baseY+160+32+80,256,32,-1,-1,"Use Preset: Bright","Use a brighter color setting. (Made by Chrisay)");
defOptionPaths(2,_fb+4,0,_fb+3,0,numTabs,0,_fb,0,-1,-1);

//ACCESSIBILITY
if global.hideTime{_ind = 1;}else{_ind = 0;}
defOptionButton(3,_fb,_baseX,_baseY,32,32,sprOptionSymbols,_ind,"Hide Timer","Ingame time will not display. I appreciate every second you spend here, though!");
defOptionPaths(3,_fb,0,0,0,_fb+1,0,-1,-1,-1,-1);
if global.runSwitch{_ind = 1;}else{_ind = 0;}
defOptionButton(3,_fb+1,_baseX,_baseY+40,32,32,sprOptionSymbols,_ind,"Auto-Run","Automatically run. Hold the run button to walk. Use if your fingers hurt.");
defOptionPaths(3,_fb+1,0,_fb,0,_fb+2,0,-1,-1,-1,-1);
if global.fullJumps{_ind = 1;}else{_ind = 0;}
defOptionButton(3,_fb+2,_baseX,_baseY+80,32,32,sprOptionSymbols,_ind,"Full Jumps","Makes jumping easier if you don't care about precision.");
defOptionPaths(3,_fb+2,0,_fb+1,0,_fb+3,0,-1,-1,-1,-1);
if global.allUnlock{_ind = 1;}else{_ind = 0;}
defOptionButton(3,_fb+3,_baseX,_baseY+120,32,32,sprOptionSymbols,_ind,"Unlock Everything","Explore the world! The warp hotkey is also enabled. Can safely disable at any time!");
defOptionPaths(3,_fb+3,0,_fb+2,0,_fb+4,0,-1,-1,-1,-1);
if global.fastAnims{_ind = 1;}else{_ind = 0;}
defOptionButton(3,_fb+4,_baseX,_baseY+160,32,32,sprOptionSymbols,_ind,"Fast Animations","Various animations become faster. Enable if things feel too slow.");
defOptionPaths(3,_fb+4,0,_fb+3,0,numTabs,0,-1,-1,-1,-1);
...
//lol. lmao
var _numControls = 18;//Could change this to un-hardcode some keybinds... Or not
for(var ix = 0; ix < 3; ix += 1){
    for(var iy = 0; iy < 6; iy += 1){
        var _ctrlID = _fb+(6*ix)+iy;
        var _ltID = _ctrlID - 6; if ix == 0{_ltID = -1;}
        var _rtID = _ctrlID + 6; if _rtID >= (_fb+_numControls){_rtID = _fb+_numControls-1;} if ix == 4{_rtID = -1;}
        var _upID = _ctrlID - 1; if iy == 0{_upID = 0;}
        var _dnID = _ctrlID + 1; if iy == 5 || (_ctrlID - _fb == (_numControls-1)){_dnID = numTabs;}
        defOptionPaths(4,_ctrlID,0,_upID,0,_dnID,0,_ltID,0,_rtID,0);
    }
}

//End by setting the current button to the Back button.
curButton = 5;
