//Draws the sliders in the pause menu.
//Only draw sliders in the current tab.
//Use current color palette.
for(var i = 0; i < tabSliders[tab]; i += 1){
    //color
    var outCol = colOut;
    var boxCol = colBox;
    if curSlider == i{
    ...
    var _sY = oSldY[tab,i];
    var _sW = oSldW[tab,i];
    var _lerpAmt = oSldPos[tab,i]/(oSldNotches[tab,i]-1);
    var _hX = _sX+round(_sW*_lerpAmt);
    
    //Back line
    draw_line_width_color(_sX,_sY,_sX+_sW,_sY,4,outCol,outCol);
    //shadow box
    draw_set_color(merge_color(boxCol,c_black,.5));
    draw_set_alpha(.35);
